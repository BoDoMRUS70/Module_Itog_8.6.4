...
namespace Module_8_6_4
{
    class Program
    {
        static void Main(string[] args)
        {
            try//Создаем папку
            {
                DirectoryInfo di = new DirectoryInfo("C:/Domeny/Students");
                if (!di.Exists)
                {
                    di.Create();
                }
            ...
            using (var fs = new FileStream("C:/Domeny/Students.dat", FileMode.OpenOrCreate))
            {
                ...
                for (int i = 0; i < student.Length; i++)
                {
                    ...
                    try//Создаем файл
                    {
                        var fileInfo = new FileInfo("C:/Domeny/Students" + student[i].Group + ".txt");
                        if (!fileInfo.Exists)
                        {
                        ...
                        else
                        {
                            ...
                            using (StreamReader sr = new StreamReader("C:/Domeny/Students" + student[i].Group + ".txt"))
                            {
                                string line;
